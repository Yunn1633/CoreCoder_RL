...
from corecoder_rl.server import CoreCoderAPIServer
...
class AsyncRolloutWorker:
    def __init__(self, args, data_buffer):
        ...
        self._server = CoreCoderAPIServer(args=args, output_queue=self.output_queue, submission_enabled=self._submission_enabled)
...
async def _drain_output_queue(args, worker: AsyncRolloutWorker) -> list[list[Sample]]:
    ...
    while len(data) < target_data_size:
        ...
        if time.time() - last_progress > 30:
            print(
                f"[CoreCoderWorker] waiting for API-produced samples: {len(data)}/{target_data_size}, "
    ...
    print(f"[CoreCoderWorker] drained {len(data)} groups in {time.time() - start:.2f}s", flush=True)
    return data


def generate_rollout_corecoder(args, rollout_id, data_buffer, evaluation=False):
    ...
    if eval_scores:
        ...
        print(
            f"[CoreCoderWorker] prm_eval_score={extra_metrics['rollout/prm_eval_score']:.4f} "
            f"(n={len(eval_scores)})",
            flush=True,
        )
