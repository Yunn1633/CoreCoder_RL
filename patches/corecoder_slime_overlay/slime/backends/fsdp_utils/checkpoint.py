...
def _cleanup_old_checkpoints(base_dir: Path, keep_step_id: int) -> None:
    if os.getenv("CORECODER_KEEP_LAST_CHECKPOINT_ONLY", "0").lower() not in {"1", "true", "yes", "on"}:
        return
    keep_name = f"iter_{keep_step_id:07d}"
    for child in base_dir.glob("iter_*"):
        if not child.is_dir() or child.name == keep_name:
            continue
        try:
            shutil.rmtree(child)
            logger.info("[FSDP] Removed old checkpoint %s", child)
        except OSError as exc:
            logger.warning("[FSDP] Failed to remove old checkpoint %s: %s", child, exc)
...
def save(actor: Any, iteration: int) -> None:
    ...
    if dist.get_rank() == 0:
        ...
        _cleanup_old_checkpoints(base_dir, step_id)
