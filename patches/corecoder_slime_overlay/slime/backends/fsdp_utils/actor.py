...
def _append_train_metrics_jsonl(args, step: int, log_dict: dict):
    path = os.getenv("CORECODER_TRAIN_METRICS_FILE", "")
    if not path:
        return
    row = {
    ...
    try:
        ...
        logger.warning("failed to append train metrics jsonl: %s", exc)
...
class FSDPTrainRayActor(TrainRayActor):
    ...
    def _log_rollout_data(self, rollout_id: int, rollout_data, packed_batches):
        ...
        for metric_key in ["log_probs", "rollout_log_probs", "teacher_log_probs", "ref_log_probs", "advantages", "returns"]:
    ...
    def _train_core(self, rollout_id: int, rollout_data) -> None:
        if self.args.advantage_estimator in ["grpo", "gspo"]:
            ...
            if "teacher_log_probs" not in rollout_data:
                raise KeyError("teacher_log_probs must be provided for on_policy_distillation")
            rollout_data["advantages"] = rollout_data["returns"] = [
    ...
    def _train_step(self, packed_batch, reported_accum, mbs_id, grad_accum):
        ...
        if self.args.advantage_estimator == "on_policy_distillation":
            missing_teacher = [
                ...
                if "teacher_log_probs" not in batch or not isinstance(batch["teacher_log_probs"], torch.Tensor)
            ]
            if missing_teacher:
                raise KeyError(f"teacher_log_probs must be provided for OPSD/OPD distillation. Missing in batches: {missing_teacher}")
            log_probs = torch.cat([batch["cur_log_probs"] for batch in unpacked_batches], dim=0)
            teacher_log_probs = torch.cat([batch["teacher_log_probs"] for batch in unpacked_batches], dim=0).to(device=log_probs.device)
            ...
            if teacher_log_probs.numel() != log_probs.numel():
                n = min(teacher_log_probs.numel(), log_probs.numel())
            ...
            loss = loss - self.args.entropy_coef * entropy_loss
            ...
            if (mbs_id + 1) in grad_accum:
                ...
                if dist.get_rank() == 0:
                    ...
                    _append_train_metrics_jsonl(self.args, self.global_step, log_dict)
